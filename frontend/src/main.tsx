...
function Campaigns({ dashboard, campaigns, approvals, targets, expanded, toggle, decide, approvalBusy, simulateOutcome, retryDelivery }: any) {
  ...
  return <div className="campaign-list"><section className="capacity-card"><div><span>Retention customers remaining</span><strong>{dashboard?.eligible_retention_customers ?? "—"}</strong></div><div><span>Retention customers targeted</span><strong>{dashboard?.currently_targeted_customers ?? "—"}</strong></div><p>Duplicate protection is scoped by campaign purpose: retention customers are not repeatedly targeted, and frequent cancellers are not repeatedly asked for feedback.</p></section>{campaigns.length === 0 && <Card title="No campaigns yet"><p className="muted">Explicitly request a campaign from the Agent workspace.</p></Card>}{campaigns.map((campaign: any) => { const approval = approvalFor(campaign.id); const rows = targets[campaign.id] || []; const delivered = (campaign.delivery?.sent || 0) + (campaign.delivery?.simulated || 0); return <section className="campaign-card" key={campaign.id}>
    ...
    {campaign.status !== "draft" && campaign.status !== "rejected" && <div className="delivery-strip"><b>Email provider status</b><span>{campaign.delivery?.sent || 0} accepted by SMTP</span><span>{campaign.delivery?.simulated || 0} simulated</span><span className={campaign.delivery?.failed ? "danger" : ""}>{campaign.delivery?.failed || 0} failed</span>{delivered === 0 && <span className="danger">No provider acceptance recorded</span>}</div>}
...
function Operations({ tasks, execute, updateTaskStatus }: any) {
  ...
  const supportOpen = supportTasks.filter((item: any) => item.status === "open");
  const supportCompleted = supportTasks.filter((item: any) => item.status === "completed");
  const productOpen = productTasks.filter((item: any) => item.status === "open");
  const productCompleted = productTasks.filter((item: any) => item.status === "completed");
  ...
  <div className="operations-queues">
    <Card title={`Open Customer Care cases (${supportOpen.length})`} eyebrow="CUSTOMER CARE WORK QUEUE"><p className="table-explainer">One internal follow-up record per qualifying customer. Completed customers remain protected from duplicate cases for this static data snapshot.</p><TaskList rows={supportOpen} onStatus={updateTaskStatus} empty="No open Customer Care cases."/><CompletedTasks title="Completed Customer Care history" rows={supportCompleted} onStatus={updateTaskStatus}/></Card>
    <Card title={`Open product recovery tasks (${productOpen.length})`} eyebrow="PRODUCT REVIEW WORK QUEUE"><p className="table-explainer">One investigation per product. Completed products remain protected from duplicate recovery tasks until new product evidence is imported.</p><TaskList rows={productOpen} onStatus={updateTaskStatus} empty="No open product recovery tasks."/><CompletedTasks title="Completed product review history" rows={productCompleted} onStatus={updateTaskStatus}/></Card>
  </div>
</div>; }
...
function CompletedTasks({ title, rows, onStatus }: any) { return <details className="completed-history"><summary>{title} ({rows.length})</summary><TaskList rows={rows} onStatus={onStatus} empty="No completed records yet."/></details>; }
...
function ProductPortfolio({ data }: any) { const PortfolioTable = ({ rows }: any) => <div className="table-wrap result-table"><table><thead><tr><th>Product</th><th title="Stock Keeping Unit: the source dataset's unique product identifier">Product code</th><th>Unit price</th><th>Cancellation rate</th><th title="Change in completed units between the newer and earlier halves of the imported dataset">Completed-sales change</th></tr></thead><tbody>{rows.map((p: any) => <tr key={p.id}><td><b>{p.name}</b></td><td>{p.sku}</td><td>{money(p.unit_price)}</td><td>{pct(p.cancellation_rate)}</td><td>{p.has_completed_sales === false ? <span className="muted">No completed sales</span> : pct(p.sales_trend)}</td></tr>)}</tbody></table></div>; return <div className="portfolio-result"><p>{data.method} <b>Product code</b> is the dataset's Stock Keeping Unit (SKU), a unique identifier used to distinguish products. <b>Completed-sales change</b> compares units sold in the newer half of the imported timeline with its earlier half. Products without completed orders are labelled separately instead of showing a misleading 0%. Median price: <b>{money(data.price_split)}</b>.</p><h3>Lower-price products with the most cancellations</h3><PortfolioTable rows={data.most_cancelled_low_value || []}/><h3>Higher-price products with the fewest cancellations</h3><PortfolioTable rows={data.high_value_low_cancellation || []}/></div>; }

...
function Result({ response, execute, openCampaigns, openOperations }: any) {
  ...
  return <section className="result-card"><div className="result-head"><div><span className="status-dot"></span><b>Request completed</b><p>{result.summary}</p></div><Badge value={response.intent || result.kind}/></div>
    ...
    {result.kind === "cancellation_customers" && customers.length > 0 && <CancellationTable rows={customers}/>}
...
function Customers({ rows, search, setSearch, execute }: any) {
  return <Card title="Customer health and value" eyebrow="LIVE POSTGRESQL DATA" action={<input className="search" value={search} onChange={event => setSearch(event.target.value)} placeholder="Search customer ID, country, or group…"/>}>
    <p className="table-explainer">Risk is an explainable inactivity score derived from time since the last purchase, with an extra warning for one-time buyers. Spending is used separately to determine the customer group. It is not a confirmed cancellation prediction.</p>
...
function Guide({ execute }: any) {
  ...
  return <div className="guide-page">
    ...
    <section><div className="section-heading"><div><p className="kicker">METRIC DEFINITIONS</p><h2>What the customer columns mean</h2></div></div><div className="definition-grid">
      ...
      <Definition title="Customer group" technical="Segment" text="A clear combination of two independent facts: risk is high at 65% or above, and spend is high at £250 or above. This produces high-spend at risk, lower-spend at risk, high-spend active, and lower-spend active groups."/>
    </div></section>
...
function CustomerTable({ rows }: any) { if (rows.some((item: any) => item.cancelled_orders !== undefined)) return null; return <div className="table-wrap result-table"><table className="customer-result-table"><thead><tr><th scope="col">Customer ID</th><th scope="col">Country</th><th scope="col">Customer group</th><th scope="col">Likelihood of not returning</th><th scope="col">Total customer spend</th></tr></thead><tbody>{rows.map((item: any) => <tr key={item.id || item.customer_id}><td><b>{item.external_id}</b></td><td>{item.country || "—"}</td><td>{item.segment ? friendlySegment(item.segment) : "—"}</td><td className="numeric">{pct(item.risk ?? item.churn_risk)}</td><td className="numeric"><b>{money(item.lifetime_value ?? item.ltv)}</b></td></tr>)}</tbody></table></div>; }
...
const friendlySegment = (value: string) => ({ at_risk_high_value: "High-spend customer at risk", at_risk_lower_value: "Lower-spend customer at risk", high_value_active: "High-spend active customer", regular_active: "Lower-spend active customer", active: "Legacy active customer", champion: "Legacy high-value customer", frequent_cancellers: "Frequent order cancellers" }[String(value)] || String(value || "").replaceAll("_", " "));

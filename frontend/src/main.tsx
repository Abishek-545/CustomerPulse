...
function ProductPortfolio({ data }: any) { const PortfolioTable = ({ rows }: any) => <div className="table-wrap result-table"><table><thead><tr><th>Product</th><th title="Stock Keeping Unit: the source dataset's unique product identifier">Product code</th><th>Unit price</th><th>Cancellation rate</th><th>Sales trend</th></tr></thead><tbody>{rows.map((p: any) => <tr key={p.id}><td><b>{p.name}</b></td><td>{p.sku}</td><td>{money(p.unit_price)}</td><td>{pct(p.cancellation_rate)}</td><td>{pct(p.sales_trend)}</td></tr>)}</tbody></table></div>; return <div className="portfolio-result"><p>{data.method} <b>Product code</b> is the dataset's Stock Keeping Unit (SKU), a unique identifier used to distinguish products. Median price: <b>{money(data.price_split)}</b>.</p><h3>Lower-price products with the most cancellations</h3><PortfolioTable rows={data.most_cancelled_low_value || []}/><h3>Higher-price products with the fewest cancellations</h3><PortfolioTable rows={data.high_value_low_cancellation || []}/></div>; }

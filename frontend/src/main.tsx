...
function Overview({ dashboard, investigations, campaigns, pending, products, execute }: any) {
  return <>
    ...
    <section className="two-col">
      ...
      <Card title="Campaign capacity" eyebrow="HUMAN-IN-THE-LOOP"><div className="campaign-summary"><strong>{dashboard?.eligible_retention_customers ?? "—"}</strong><span>customers still eligible</span><strong>{dashboard?.currently_targeted_customers ?? "—"}</strong><span>customers targeted historically</span></div><p className="muted">Every campaign selects customers who have never appeared in an earlier campaign. With this static demo dataset, the pool eventually reaches zero; importing newer data or adopting a documented re-engagement policy can replenish it.</p></Card>
    </section>
  </>;
}
